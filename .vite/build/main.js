...
electron.ipcMain.on("electron-store-get", async (event, val) => {
...
async function resolveGraphicBackgroundPath() {
  const userPath = path$1.join(electron.app.getPath("userData"), customBackgroundFileName);
  return { path: userPath, location: "userData" };
}
...
electron.ipcMain.handle("save-custom-background", async (event, dataUrl) => {
  try {
...
electron.ipcMain.handle("load-custom-background", async () => {
  try {
    const info = await resolveGraphicBackgroundPath();
    const buffer = await fs$1.readFile(info.path);
    const base64Content = buffer.toString("base64");
...
electron.ipcMain.handle("get-custom-background-path", async () => {
  const info = await resolveGraphicBackgroundPath();
  return info.path;
});
